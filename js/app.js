...
function canonicalSourceText(v) {
    ...
    if (['local store', 'local purchase', 'local'].includes(t))
        return 'Local Purchase';
    if (['direct purchase', 'head office', 'pre-ordered', 'head office purchase', 'headoffice purchase'].includes(t))
        return 'Head Office Purchase';
    return String(v || '');
}
...
// Add Modals Toggles
function openAddModal() { addModal.classList.remove('hidden'); if (typeof populateJobLinkSelect === 'function')
    populateJobLinkSelect(); }
...
function calculateDateGap(reqDate, recDate) {
    ...
    if (diffDays < 0)
        return `<span class="text-emerald-600 dark:text-emerald-400 font-bold">${Math.abs(diffDays)} days early</span>`;
    if (diffDays === 0)
        return `<span class="text-indigo-650 dark:text-indigo-400 font-bold">Same day</span>`;
...
function getStatusBadge(reqQty, recQty, reqDateStr) {
    ...
    if (recQty === 0)
        return '<span class="px-2.5 py-1 inline-flex text-[10px] leading-tight font-extrabold rounded-full bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-900/30 shadow-sm">Pending</span>';
    if (recQty < reqQty)
        return '<span class="px-2.5 py-1 inline-flex text-[10px] leading-tight font-extrabold rounded-full bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-900/30 shadow-sm">Partial</span>';
    if (recQty === reqQty)
        return '<span class="px-2.5 py-1 inline-flex text-[10px] leading-tight font-extrabold rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30 shadow-sm">Completed</span>';
    if (recQty > reqQty)
        return '<span class="px-2.5 py-1 inline-flex text-[10px] leading-tight font-extrabold rounded-full bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400 border border-purple-100 dark:border-purple-900/30 shadow-sm">Over-received</span>';
}
...
async function fetchTrackerPage() {
    try {
        ...
        if (!res.ok)
            throw new Error('Failed to load tracker page');
...
async function loadAllData() {
    try {
        ...
        if (!res.ok)
            throw new Error('Failed to load full database list');
...
function handleAdvSearchChange() {
    ...
    if (advCat)
        searchCategory = advCat.value;
    const advSrc = document.getElementById('advRequestSource');
    if (advSrc)
        searchRequestSource = advSrc.value;
    currentPage = 1;
    renderCategoryChips();
    fetchTrackerPage();
}
...
async function loadVehicles() {
    ...
    if (dl)
        dl.innerHTML = allVehicles.map(v => `<option value="${escapeHtml(v)}"></option>`).join('');
...
async function loadIssues() {
    try {
        ...
        if (issueFilters.search)
            p.set('search', issueFilters.search);
        if (issueFilters.vehicle && issueFilters.vehicle !== 'all')
            p.set('vehicle', issueFilters.vehicle);
        if (issueFilters.category && issueFilters.category !== 'all')
            p.set('category', issueFilters.category);
        if (issueFilters.startDate)
            p.set('startDate', issueFilters.startDate);
        if (issueFilters.endDate)
...
function setupIssueDesk(type = null, id = null) {
    ...
    searchInput.oninput = () => {
        ...
        const match = activeItems.find(item => `${item.mrnNum} - ${item.name} (${item.vehicleMachinery})` === val);
        if (match) {
...
function updateSidebarBadges() {
    ...
    displayAll.forEach(item => {
        ...
        if (!isPendingDelivery && isPendingPricing)
            countPendingPricing++;
    });
    ...
    if (allBadge)
        allBadge.textContent = countAll;
    if (delBadge)
        delBadge.textContent = countPendingDelivery;
    if (prBadge)
        prBadge.textContent = countPendingPricing;
    ...
    if (invBadge)
        invBadge.textContent = inStockBadgeCount;
...
function setInventoryStatusFilter(filter) {
    ...
    btns.forEach(b => {
        ...
        if (b === filter) {
            ...
            if (b === 'instock')
                borderHoverClass = 'hover:border-emerald-305 dark:hover:border-emerald-900/50';
            if (b === 'lowstock')
                borderHoverClass = 'hover:border-amber-305 dark:hover:border-amber-900/50';
            if (b === 'outstock')
                borderHoverClass = 'hover:border-rose-305 dark:hover:border-rose-900/50';
...
function renderPricingSummary() {
    ...
    if (totalSpendValEl)
        totalSpendValEl.textContent = formatCurrency(Math.round(totalSpend * 100) / 100);
    if (supplierCountValEl)
        supplierCountValEl.textContent = Object.keys(supplierSpend).length;
    if (pricedItemsValEl)
        pricedItemsValEl.textContent = pricedCount;
    if (unpricedItemsValEl)
        unpricedItemsValEl.textContent = unpricedReceivedCount;
...
function renderFleetView() {
    ...
    const allVehicleNames = new Set();
    displayAll.forEach(item => {
        if (item.vehicleMachinery)
            allVehicleNames.add(item.vehicleMachinery.trim());
    });
    issues.forEach(is => {
        if (is.vehicleMachinery)
            allVehicleNames.add(is.vehicleMachinery.trim());
    });
...
function renderVehicleOffcanvasData() {
    ...
    vehicleItems.forEach(item => {
        ...
        if (isPendingSupplier) {
            pendingSupplierCount++;
            if (parseDate(item.reqDate) < today)
                hasOverdue = true;
        }
        if (isPendingWorkshop) {
            pendingWorkshopCount++;
...
function setupReceivingDesk(prefilledId = null) {
    ...
    if (prefilledId) {
        ...
        recQtyInput.value = '';
        if (filenameLabel)
            filenameLabel.textContent = 'No invoice PDF selected (optional)';
    }
...
function closePricingAuditWorkspace() {
    ...
    if (!formContainer || !emptyContainer)
        return;
    if (auditReceiptIdInput)
        auditReceiptIdInput.value = '';
...
function handleRouting() {
    ...
    document.querySelectorAll('#sidebarNav a').forEach(link => {
        ...
        if (link.id === 'nav-' + currentView || isBatterySubView || isTransferSubView) {
            ...
            if (badge)
                badge.className = "sidebar-badge bg-indigo-100 dark:bg-indigo-900/60 text-indigo-850 dark:text-indigo-300 py-0.5 px-3 rounded-full text-xs font-extrabold transition-all";
        }
        else {
            ...
            const badge = link.querySelector('.sidebar-badge');
            if (badge)
                badge.className = "sidebar-badge bg-slate-100 dark:bg-slate-800 text-slate-655 dark:text-slate-400 py-0.5 px-3 rounded-full text-xs font-extrabold transition-all";
        }
    });
...
editRequestDetailsForm.addEventListener('submit', async (e) => {
    ...
    await syncService.enqueue('UPDATE_ITEM', `/api/items/${itemId}`, 'PUT', updateData, () => {
...
addForm.addEventListener('submit', async (e) => {
    ...
    const newItem = { mrnNum, reqDate, vehicleMachinery, itemName, itemDesc, reqQty, category, requestSource };
    if (jobCardId)
        newItem.jobCardId = jobCardId;
    await syncService.enqueue('CREATE_ITEM', '/api/items', 'POST', newItem, (tempId) => {
        const localItem = {
            id: tempId,
            mrnNum,
            reqDate,
...
receivingDeskForm.addEventListener('submit', async (e) => {
    ...
    await syncService.enqueue('CREATE_RECEIPT', `/api/items/${itemId}/receipts`, 'POST', receiptEntry, (tempRecId) => {
        const item = allItems.find(i => String(i.id) === String(itemId));
        if (item) {
            item.receipts = item.receipts || [];
            item.receipts.push({
...
document.querySelectorAll('input[name="recDeskPurchaseSource"]').forEach(r => r.addEventListener('change', updateRecDeskMismatchNote));
...
pricingAuditForm.addEventListener('submit', async (e) => {
    ...
    await syncService.enqueue('UPDATE_RECEIPT', `/api/receipts/${receiptId}`, 'PUT', updateData, () => {
        const item = allItems.find(i => String(i.id) === String(itemId));
...
offcanvasPricingForm.addEventListener('submit', async (e) => {
    ...
    if (receiptIdx === '')
        return;
    const item = allItems.find(i => String(i.id) === String(itemId));
    if (!item || !item.receipts[receiptIdx])
        return;
    const receipt = item.receipts[receiptIdx];
    ...
    const unitPrice = isNaN(unitPriceVal) ? null : unitPriceVal;
    const updateData = { grnNumber, invoiceNumber, invoiceDate, supplierName, unitPrice };
    await syncService.enqueue('UPDATE_RECEIPT', `/api/receipts/${receipt.id}`, 'PUT', updateData, () => {
        Object.assign(receipt, updateData);
        recalcItemReceipts(item);
        const pageItem = items.find(i => String(i.id) === String(itemId));
        if (pageItem && pageItem.receipts) {
...
function toggleLedgerShowAll() {
    ...
    if (btnText)
        btnText.textContent = ledgerShowAll ? 'Show Less' : 'Show All Days';
    if (btnIcon) {
        if (ledgerShowAll)
    ...
    if (lines.length > 1) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (newItem.recQty > 0) {
                ...
                if (grnVal)
                    receipt.grnNumber = grnVal;
                if (invNumVal)
                    receipt.invoiceNumber = invNumVal;
                if (invDateVal)
                    receipt.invoiceDate = invDateVal;
                if (supplierVal)
                    receipt.supplierName = supplierVal;
                if (unitPriceVal !== null && !isNaN(unitPriceVal))
                    receipt.unitPrice = unitPriceVal;
...
async function processReceivingDeskPdfAutoFill(file) {
    try {
        ...
        if (!res.ok)
            throw new Error('PDF scanning failure');
        const result = await res.json();
        if (result.success && result.data) {
            const d = result.data;
            if (d.reqQty)
                document.getElementById('recDeskQty').value = d.reqQty;
            if (d.reqDate)
                document.getElementById('recDeskDate').value = d.reqDate;
...
setInterval(async () => {
    ...
    const isEditing = (addModal && !addModal.classList.contains('hidden')) ||
    ...
    if (!document.hidden && !isEditing && !hasFocus) {
        if (syncService && syncService.queue.length > 0) {
            if (!syncService.isProcessing) {
                ...
                if (hasError) {
                    syncService.queue.forEach(q => { if (q.retries > 5)
                        q.retries = 5; });
...
async function updateBatteryKPIs() {
    try {
        ...
        if (res.ok) {
            ...
            if (badge)
                badge.textContent = stats.newInStore + stats.oldInStore;
        }
    }
    catch (err) {
    ...
    try {
        ...
        if (movements.length === 0) {
            ...
            chronologicalMovements.forEach((m, idx) => {
                ...
                if (m.conditionAfter === 'Old')
                    dotColor = "bg-amber-500";
                else if (m.conditionAfter === 'Expired')
                    dotColor = "bg-rose-500";
                else if (m.conditionAfter === 'New')
...
async function deleteBatteryRecord(id) {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(r.error || 'Deletion failed');
        await fetchBatteries();
        renderBatteriesView();
        updateSidebarBadges();
    }
    ...
    if (moveForm) {
        moveForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const r = await res.json();
                if (!res.ok)
                    throw new Error(r.error || 'Movement logging failed');
                await fetchBatteries();
                renderBatteriesView();
                updateSidebarBadges();
                window.location.hash = '#batteries';
    ...
    if (transferForm) {
        transferForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const r = await res.json();
                if (!res.ok)
                    throw new Error(r.error || 'Save transfer failed');
                await fetchTransfers();
                renderTransfersView();
                updateSidebarBadges();
...
async function renderJobCards() {
    ...
    if (badge)
        badge.textContent = rows.filter((r) => r.status !== 'CLOSED').length;
    if (!rows.length) {
...
async function openJobCardDetail(id) {
    ...
    let jc;
    try {
        jc = await (await fetch('/api/jobcards/' + id)).json();
    }
    catch (e) {
        jc = null;
    }
    if (!jc || jc.error) {
        mount.innerHTML = `<div class="py-10 text-center text-sm font-semibold text-rose-500">Could not load this job card.</div>`;
        return;
    }
...
function renderJobCardModal(jc) {
    ...
    mount.innerHTML = `
            ...
            `;
    if (typeof renderJobProgramme === 'function')
        renderJobProgramme(jc); // Phase 3 hook
    if (typeof renderJobMrns === 'function')
        renderJobMrns(jc); // Phase 4 hook
    if (typeof renderJobIssues === 'function')
        renderJobIssues(jc); // Addendum 3 hook
}
...
function toggleVendorField() {
    ...
    if (wrap)
        wrap.classList.toggle('hidden', type !== 'OUTSOURCED');
}
async function setupJobCardEntry(param) {
    const form = document.getElementById('jobCardForm');
    if (form)
        form.reset();
    const err = document.getElementById('jcEntryError');
    if (err)
    ...
    if (param) {
        ...
        if (sub)
            sub.textContent = 'Update the job card details.';
        try {
        ...
        if (title)
            title.textContent = 'New Job Card';
        if (sub)
            sub.textContent = 'Create a vehicle / machinery repair or service job.';
        const d = document.getElementById('jcDate');
        if (d && !d.value)
...
async function saveJobCard(event) {
    ...
    if (!payload.vehicleMachinery.trim()) {
        if (err) {
            err.textContent = 'Vehicle / Machinery is required.';
            err.classList.remove('hidden');
        }
        return;
    }
    const btn = document.getElementById('jcSaveBtn');
    if (btn)
        btn.disabled = true;
    try {
...
function editJobDaily(id) {
    const e = (window.__jcProgramme || {})[id];
    if (!e)
        return;
...
async function loadJobOptions(selectedId) {
    const sel = document.getElementById('dpqJob');
    if (!sel)
        return;
    let data;
    try {
        data = await (await fetch('/api/jobcards?limit=300')).json();
    }
    catch (e) {
        return;
    }
...
async function renderProgramme() {
    ...
    const body = document.getElementById('programmeTodayBody');
    if (!body)
        return;
    body.innerHTML = `<tr><td colspan="5" class="px-5 py-8 text-center text-sm font-semibold text-slate-400">Loading…</td></tr>`;
    let data;
    try {
        data = await (await fetch('/api/programme?dateISO=' + encodeURIComponent(dateISO))).json();
    }
    catch (e) {
        return;
    }
    const entries = data.programme || [];
    window.__todayProgramme = {};
    entries.forEach((e) => { window.__todayProgramme[e.id] = e; });
    let hrs = 0, lab = 0;
...
function resetDailyForm() {
    ['dpqEditId', 'dpqHours', 'dpqOutside', 'dpqMechanics', 'dpqWork', 'dpqVehicle'].forEach((id) => { const el = document.getElementById(id); if (el)
        el.value = ''; });
    const mi = document.getElementById('dpqMatchInfo');
    if (mi)
        mi.textContent = '';
    const b = document.getElementById('dpqEditBadge');
    if (b)
        b.classList.add('hidden');
    const c = document.getElementById('dpqCancel');
    if (c)
        c.classList.add('hidden');
    const h = document.getElementById('dpqHeading');
    if (h)
        h.textContent = "Add Today's Work";
}
let dpqMatchTimer = null;
function dpqMatchPreview() {
    clearTimeout(dpqMatchTimer);
    dpqMatchTimer = setTimeout(async () => {
        const info = document.getElementById('dpqMatchInfo');
        if (!info)
            return;
        ...
        if ((document.getElementById('dpqJob') || {}).value) {
            info.innerHTML = '<span class="text-slate-400">Using the selected job (override).</span>';
            return;
        }
        if (!vehicle) {
            info.textContent = '';
        ...
        try {
            ...
            if (data.match)
                info.innerHTML = '→ matches <span class="text-emerald-600 dark:text-emerald-400 font-bold">' + jcEsc(data.match.jobNo) + '</span>';
            else
                info.innerHTML = '→ <span class="text-amber-600 font-bold">no job in window — will file under DW-' + jcEsc(vehicle.replace(/\s+/g, '').toUpperCase()) + '</span>';
        }
        catch (e) {
            info.textContent = '';
        }
...
function editDailyEntry(id) {
    const e = (window.__todayProgramme || {})[id];
    if (!e)
        return;
    ...
    const h = document.getElementById('dpqHeading');
    if (h)
        h.textContent = 'Edit Entry';
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
async function saveDailyEntry() {
    const err = document.getElementById('dpqError');
    if (err)
        err.classList.add('hidden');
    ...
    const vehicle = ((document.getElementById('dpqVehicle') || {}).value || '').trim();
    const dateISO = (document.getElementById('dpViewDate') || {}).value || dpTodayISO();
    if (!editId && !jobCardId && !vehicle) {
        if (err) {
            err.textContent = 'Enter a vehicle (or pick a job).';
            err.classList.remove('hidden');
        }
        return;
    }
    ...
    try {
        let res;
        if (editId)
            res = await fetch('/api/programme/' + editId, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        else if (jobCardId)
            res = await fetch('/api/jobcards/' + jobCardId + '/programme', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        else
            res = await fetch('/api/programme/auto', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.assign({}, payload, { vehicle })) });
        const data = await res.json();
        if (!res.ok) {
            if (err) {
                err.textContent = data.error || 'Could not save.';
...
async function loadMechanicDatalist() {
    let data;
    try {
        data = await (await fetch('/api/mechanics')).json();
    }
    catch (e) {
        return;
    }
    ...
    if (dl)
        dl.innerHTML = active.map((m) => `<option value="${jcEsc(m.name)}">`).join('');
}
...
async function loadItemNameDatalist() {
    let data;
    try {
        data = await (await fetch('/api/item-names')).json();
    }
    catch (e) {
        return;
    }
    ...
    if (dl)
        dl.innerHTML = (data.names || []).map((n) => `<option value="${jcEsc(n)}">`).join('');
}
function openMechanicsModal() { const m = document.getElementById('mechanicsModal'); if (m)
    m.classList.remove('hidden'); renderMechanicsList(); }
function closeMechanicsModal() { const m = document.getElementById('mechanicsModal'); if (m)
    m.classList.add('hidden'); }
async function renderMechanicsList() {
    const body = document.getElementById('mechanicsListBody');
    if (!body)
        return;
    let data;
    try {
...
async function saveMechanic(id, active) {
    ...
    try {
        await fetch('/api/mechanics/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hourlyRate: rate, active }) });
        await loadMechanicDatalist();
    }
    catch (e) { }
}
async function addMechanic() {
    ...
    try {
        ...
        if (res.ok) {
            document.getElementById('newMechName').value = '';
            document.getElementById('newMechRate').value = '';
            renderMechanicsList();
            loadMechanicDatalist();
        }
        else {
            const d = await res.json();
...
if (document.readyState === 'loading')
    document.addEventListener('DOMContentLoaded', loadJobModalLookups);
else
    loadJobModalLookups();
...
function renderJobMrns(jc) {
    ...
    const rows = items.map((it) => {
        ...
        const badges = (it.notReceived ? `<span class="px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wider bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400">Pending ${it.recQty || 0}/${it.reqQty || 0}</span>` : '') +
    ...
    const counts = (jc.pendingCount ? ` · <span class="text-amber-600">${jc.pendingCount} pending</span>` : '') +
...
async function loadLinkableMrns(jobCardId) {
    ...
    let data;
    try {
        data = await (await fetch('/api/jobcards/' + jobCardId + '/linkable-mrns')).json();
    }
    catch (e) {
        return;
    }
...
async function addJobItem(jobCardId) {
    ...
    try {
        ...
        if (!res.ok) {
            if (err) {
                err.textContent = data.error || 'Could not add item.';
                err.classList.remove('hidden');
            }
            return;
        }
...
async function autoLinkAllMrns(btn) {
    ...
    try {
        ...
        if (!res.ok)
            alert(data.error || 'Could not auto-link.');
        else {
            alert('Linked ' + data.linked + ' MRNs and ' + (data.issuesLinked || 0) + ' issued items to their jobs by vehicle + date.');
            if (typeof renderJobCards === 'function')
                renderJobCards();
        }
    }
...
async function addJobIssue(jobCardId) {
    ...
    const err = document.getElementById('jisError');
    if (err)
        err.classList.add('hidden');
    if (!name) {
        if (err) {
    ...
    try {
        ...
        if (!res.ok) {
            if (err) {
                err.textContent = data.error || 'Could not add.';
                err.classList.remove('hidden');
            }
            return;
        }
...
async function unlinkIssue(issueId, jobCardId) {
    try {
        await fetch('/api/issues/' + issueId + '/link', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobCardId: null }) });
        openJobCardDetail(jobCardId);
    }
    catch (e) { }
}
async function linkMrnToJob(jobCardId) {
    ...
    try {
        ...
        if (!res.ok) {
            if (err) {
                err.textContent = data.error || 'Could not link.';
                err.classList.remove('hidden');
            }
            return;
        }
...
async function unlinkItem(itemId, jobCardId) {
    try {
        await fetch('/api/items/' + itemId + '/link', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobCardId: null }) });
        openJobCardDetail(jobCardId);
    }
    catch (e) { }
}
async function populateJobLinkSelect() {
    const sel = document.getElementById('jcLinkSelect');
    if (!sel)
        return;
    let data;
    try {
...
async function loadDashCategories() {
    ...
    try {
        ...
        dashCatsLoaded = false;
    }
}
function dashSetPresetUI() {
    ['Month', 'Year', 'All'].forEach((p) => {
        const el = document.getElementById('dashPreset' + p);
        if (!el)
            return;
...
function dashPreset(p) {
    ...
    if (p === 'month') {
        start.value = dMonthStart();
        end.value = dToday();
    }
    else if (p === 'year') {
        start.value = dYearStart();
        end.value = dToday();
    }
    else {
        start.value = '';
...
function dashReset() {
    ['dashStart', 'dashEnd', 'dashSource', 'dashCategory', 'dashVehicle', 'dashSupplier'].forEach((id) => { const el = document.getElementById(id); if (el)
        el.value = ''; });
    dashActivePreset = 'all';
    dashSetPresetUI();
    renderUnifiedDashboard();
}
...
function dashQuery() {
    ...
    const q = new URLSearchParams();
    if (g('dashStart'))
        q.set('startDate', g('dashStart'));
    if (g('dashEnd'))
        q.set('endDate', g('dashEnd'));
    if (g('dashSource'))
        q.set('source', g('dashSource'));
    if (g('dashCategory'))
        q.set('category', g('dashCategory'));
    if (g('dashVehicle'))
        q.set('vehicle', g('dashVehicle').trim());
    if (g('dashSupplier'))
        q.set('supplier', g('dashSupplier').trim());
    return q;
}
async function renderUnifiedDashboard() {
    ...
    let o;
    try {
        o = await (await fetch('/api/dashboard?' + dashQuery().toString())).json();
    }
    catch (e) {
        return;
    }
    ...
    if (bHo)
        bHo.style.width = pct(rc.headOffice) + '%';
    if (bLo)
        bLo.style.width = pct(rc.local) + '%';
    if (bOt)
        bOt.style.width = pct(rc.other) + '%';
    set('dashHo', dCur(rc.headOffice));
    set('dashLocal', dCur(rc.local));
    set('dashOther', dCur(rc.other));
    // Supplier distribution
    const sup = o.suppliers || [];
    const listEl = document.getElementById('dashSupplierList');
    if (listEl)
        listEl.innerHTML = sup.length ? sup.slice(0, 6).map((s, i) => `
    ...
    if (ctx && typeof Chart !== 'undefined') {
        if (dashSupplierChartInstance)
            dashSupplierChartInstance.destroy();
    ...
    const daily = o.daily || [];
    if (body)
        body.innerHTML = daily.length ? daily.map((d) => `
    ...
    if (hint)
        hint.textContent = daily.length ? `${daily.length} day${daily.length !== 1 ? 's' : ''}` : '';
    ...
    if (mctx && typeof Chart !== 'undefined') {
        if (dashMonthlyChartInstance)
            dashMonthlyChartInstance.destroy();
    ...
    if (mBody)
        mBody.innerHTML = (o.monthly || []).length ? o.monthly.map((m) => `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
