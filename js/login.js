...
$('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!res.ok) {
            showErr($('loginError'), data.error || 'Sign in failed.');
        ...
        showErr($('loginError'), 'Network error — please try again.');
        $('loginBtn').disabled = false;
    }
});
$('changeForm').addEventListener('submit', async (e) => {
    ...
    if ($('newPassword').value !== $('confirmPassword').value) {
        showErr($('changeError'), 'Passwords do not match.');
        return;
    }
    ...
    try {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
            showErr($('changeError'), data.error || 'Could not update password.');
        ...
        showErr($('changeError'), 'Network error — please try again.');
        $('changeBtn').disabled = false;
    }
});
