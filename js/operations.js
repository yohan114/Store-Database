...
async function opAddUserPrompt() {
    ...
    if (!ok) {
        alert((body && body.error) || 'Could not add user.');
        return;
    }
    alert('User created with temporary password: changeme123');
    opLoadUsers();
}
async function opResetPwd(id) {
    const pwd = prompt('New temporary password (min 6 chars):', 'changeme123');
    if (!pwd)
        return;
