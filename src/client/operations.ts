...
function applyRoleScopedNav(user) {
    ...
    if (opsNav) opsNav.classList.toggle('hidden', !(isAdmin || isOps));
    ...
    if (isOps && !isAdmin) {
        ...
        if (!location.hash || location.hash === '#dashboard') location.hash = '#operations';
    }
}
...
async function refreshNotifications() {
    try {
        ...
        if (badge) { badge.textContent = body.unread; badge.classList.toggle('hidden', !body.unread); }
        const list = document.getElementById('notifList');
        if (list) list.innerHTML = (body.notifications || []).length ? body.notifications.map((n) => `
...
function toggleNotifDropdown() {
    ...
    if (d) { d.classList.toggle('hidden'); if (!d.classList.contains('hidden')) refreshNotifications(); }
}
...
async function openNotif(id, requestId) {
    ...
    const d = document.getElementById('notifDropdown'); if (d) d.classList.add('hidden');
    refreshNotifications();
    if (requestId) openJobRequestDetail(requestId);
}
document.addEventListener('click', function (e) {
    const c = document.getElementById('notifContainer'); const d = document.getElementById('notifDropdown');
...
function opsDebouncedSearch() { clearTimeout(opsSearchTimer); opsSearchTimer = setTimeout(() => { opsSearchTerm = (document.getElementById('opsSearch') as any).value.trim(); loadOperations(); }, 250); }

async function loadOperations() {
    if (!opsMeta) { const m = await opJSON('/api/job-requests/meta'); opsMeta = m.body || {}; }
    // New-request button only for those who can create.
    const newBtn = document.getElementById('opsNewBtn'); if (newBtn) newBtn.classList.toggle('hidden', !opsMeta.canCreate);
    // Build query from the active filter.
    const q = new URLSearchParams();
    if (opsFilter === 'pending') q.set('pending', '1');
    else if (opsFilter === 'mine') q.set('mine', '1');
    else if (opsFilter !== 'all') q.set('status', opsFilter);
    if (opsSearchTerm) q.set('search', opsSearchTerm);
    const { body } = await opJSON('/api/job-requests?' + q.toString());
    const data = body || { requests: [], counts: {} };
    const c = data.counts || {};
    const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
    set('opsKpiPending', c.pendingMine || 0); set('opsKpiMine', c.mineOpen || 0);
    set('opsKpiProgress', c.inProgress || 0); set('opsKpiCompleted', c.completedMonth || 0);
    const pill = document.getElementById('count-ops-pending');
    if (pill) { pill.textContent = c.pendingMine || 0; pill.classList.toggle('hidden', !(c.pendingMine)); }

    // Filter chips.
    const fWrap = document.getElementById('opsFilters');
    if (fWrap) fWrap.innerHTML = OPS_FILTERS.map(([k, lbl]) =>
        `<button onclick="opsSetFilter('${k}')" class="px-3 py-1.5 rounded-lg text-xs font-bold ${opsFilter === k ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}">${lbl}</button>`).join('');

    // Rows.
    const body2 = document.getElementById('opsListBody');
    const rows = data.requests || [];
    if (body2) body2.innerHTML = rows.length ? rows.map((r) => `
...
function opCloseModal() { const m = document.getElementById('jobRequestModal'); if (m) m.classList.add('hidden'); }
function opShowModal(html) {
    const body = document.getElementById('jobRequestModalBody'); if (body) body.innerHTML = html;
    const m = document.getElementById('jobRequestModal'); if (m) m.classList.remove('hidden');
}
...
function opToggleVendor() {
    const t = (document.getElementById('jrType') as any).value;
    const w = document.getElementById('jrVendorWrap'); if (w) w.classList.toggle('hidden', t !== 'OUTSOURCED');
}
async function submitJobRequest(submit) {
    const val = (id) => { const el = document.getElementById(id) as any; return el ? el.value.trim() : ''; };
    const recips = Array.from(document.querySelectorAll('.opRecip')).filter((c: any) => c.checked).map((c: any) => c.value);
    ...
    if (!payload.title) { if (err) { err.textContent = 'A job title is required.'; err.classList.remove('hidden'); } return; }
    ...
    if (!ok) { if (err) { err.textContent = (body && body.error) || 'Could not save.'; err.classList.remove('hidden'); } return; }
    opCloseModal(); loadOperations(); refreshNotifications();
    if (body.request && body.request.id) openJobRequestDetail(body.request.id);
}

async function openJobRequestDetail(id) {
    if (location.hash !== '#operations') location.hash = '#operations';
    const { ok, body } = await opJSON('/api/job-requests/' + id);
    if (!ok || !body) return;
    const r = body;
    const actions = (r.availableActions || []).map((a) =>
        `<button onclick="jobRequestAction(${r.id}, '${a.action}', ${a.needNote ? 'true' : 'false'})" class="px-4 py-2 rounded-xl text-xs font-bold ${a.action.includes('Reject') || a.action === 'tmReject' || a.action === 'omReject' ? 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400' : 'bg-gradient-to-r from-indigo-600 to-indigo-800 text-white'}">${opEsc(a.label)}</button>`).join('');
...
async function jobRequestAction(id, action, needNote) {
    let note = '';
    if (needNote) { note = prompt('Reason:') || ''; if (!note.trim()) return; }
    ...
    if (!ok) { alert((body && body.error) || 'Action failed.'); return; }
    openJobRequestDetail(id); loadOperations(); refreshNotifications();
}
...
function renderOpsAdmin() {
    ...
    opLoadUsers(); opLoadOutbox();
    const cc = document.getElementById('opsStandingCc') as any; if (cc) cc.value = (opsMeta && opsMeta.standingCc) || '';
}
async function opLoadUsers() {
    const { body } = await opJSON('/api/users'); const mount = document.getElementById('opsUsersList'); if (!mount) return;
...
async function opLoadOutbox() {
    const { body } = await opJSON('/api/outbox'); const mount = document.getElementById('opsOutbox'); if (!mount) return;
...
async function opSaveStandingCc() {
    ...
    if (ok && opsMeta) opsMeta.standingCc = v;
