...
function main() {
    ...
    // Repair: take the median year of the 10 nearest rows by id that have a
    // sane ISO date, and rebuild the ISO with it. Bounded to ISO years < 2020.
    ...
    for (const [table, col, isoCol] of SUSPECT) {
        const bad = db.all(`SELECT id, ${col} AS d, ${isoCol} AS iso FROM ${table}
                            WHERE ${isoCol} <> '' AND ${isoCol} < '2020'`);
        let repaired = 0;
        for (const r of bad) {
            const neighbours = db.all(
                `SELECT ${isoCol} AS iso FROM ${table}
                 WHERE ${isoCol} >= '2020' AND ${isoCol} <> '' AND id BETWEEN ? AND ? AND id <> ?
                 ORDER BY ABS(id - ?) LIMIT 10`,
                [r.id - 200, r.id + 200, r.id, r.id]);
            if (!neighbours.length) { console.log(`  ${table} id=${r.id} "${r.d}" (${r.iso}) — no dated neighbours, left as-is`); continue; }
            ...
            repaired += 1;
            if (COMMIT) db.run(`UPDATE ${table} SET ${isoCol}=? WHERE id=?`, [fixedISO, r.id]);
            else if (repaired <= 8) console.log(`  ${table} id=${r.id} "${r.d}": ${r.iso} -> ${fixedISO}`);
        }
        console.log(`  ${table}.${isoCol}: ${bad.length} suspect, ${repaired} ${COMMIT ? 'repaired' : 'repairable'}`);
    }
